...
export default class LatentHistogram {
  ...
  drawLatent(ctx, unit) {
    ...
    for (let i = 0; i < dims; i += 1) {
      ...
      if (i === selectedLatent && this.showText) {
        ...
        let yTextPos = -1 * (40 + radius * 0.08) * this.graphWidth / 500;
        let textGap = 5;
  ...
  drawDiagram(ctx, w, h) {
    const { selectedLatent, frameCount } = this.renderer;
    const unit = w / this.dims;
    const width = 1;

    ctx.save();
    ctx.fillStyle = '#888';
    ctx.translate(-0.5 * w, 0);
    ...
    for (let i = 0; i < this.dims; i += 1) {
      ...
      const radius = value * this.graphRadiusRatio * unit + this.graphRadius;
      const h = -this.histogramHeightRatio * this.graphHeight * this.latent[i];
